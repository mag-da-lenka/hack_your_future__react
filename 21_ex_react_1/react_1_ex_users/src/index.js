...
class SayHiToMultiplePeopleClass extends React.Component {
    ...
    render() {
        ...
        const multiplePeople = this.props.people;         // people could be potatoes or squirrels
        const result = multiplePeople.map(person => {
            return <HelloUser name={person.name} age={person.age} />;
        });

        return <ul> result from SayHiToMultiplePeopleClass, <br></br>
            which returns the HelloUser function!!! {result} </ul>;
...
component-based - components are buit independently/separately/theu don't know about each other
